...
5. **Status Bar Quota Indicator & Rich Dashboard Tooltip**:
   - **Status Bar**: Renders bundled offline **Datatype pie chart icons** with exact 1% granularity (`$(copilot-bridge-p<pct>) <pct>%`) for percentage plans (Z.ai, Kimi) or exact balance (e.g. `¥299.79`) for currency models. Supports **Auto-Select active provider mode** or explicit provider pinning.
...
6. **Diagnostics & Factory Reset**: Built-in connectivity testing command and a 1-click factory reset command (`Copilot Bridge: Reset All Configuration & Clear Secrets`) to wipe extension secrets, reset global state, and purge bridge model/MCP configurations safely.
...
When using a text-only model (like `GLM-5.3` or `DeepSeek V4 Pro`), the built-in **Vision Agent Tool** (`copilot_bridge_analyze_visual`) delegates image analysis to any of the following multimodal backends:
...
Switch your preferred vision model at any time with **`Copilot Bridge: Select Vision Agent Model`**.
...
Copilot Bridge configures models and tools for VS Code's native BYOK engine (`chatLanguageModels.json` and `mcp.json`). VS Code communicates directly with your providers without proxying. Tool execution and multimodal routing follow standard VS Code Copilot rules:
...
- **Dedicated Companion MCP Search & Extraction Tools**:
  - Installing companion MCP tool presets via `Copilot Bridge: Configure MCP Tools` adds direct-to-provider search pipelines:
...
- **Text-Only Coding Models (`vision: false`) + Vision Agent Tool**:
  ...
  - The text-only model automatically invokes the built-in **Vision Agent Tool** (`copilot_bridge_analyze_visual`), which queries your configured vision backend (`GLM-4.6V`, `Gemini 2.5 Flash`, etc.) and feeds structured OCR and layout analysis back into the coding session within its full context window (up to 1M on GLM-5.3 and DeepSeek V4 Pro, ~200K on GLM-4.7, 128K on Qwen 3.6 Flash).

## Commands

- **Copilot Bridge: Quick Setup (Models & MCP Tools)** — run the first-time setup wizard to configure multiple providers and companion MCP tools in one pass.
- **Copilot Bridge: Add Model** — pick a single provider, select models, and optionally install companion MCP tools.
- **Copilot Bridge: Remove Model** — select an installed provider group and remove it.
- **Copilot Bridge: List Models** — inspect installed models, endpoints, and token limits.
- **Copilot Bridge: Configure MCP Tools** — select target (User Profile Global or Workspace `.vscode/mcp.json`) and choose MCP presets grouped by provider.
- **Copilot Bridge: Remove MCP Server** — choose an MCP server to remove from configuration.
- **Copilot Bridge: Configure Usage API Key** — securely store or update provider keys in extension SecretStorage for live status bar quota polling.
- **Copilot Bridge: Select Pinned Status Bar Provider** — switch which provider's usage badge appears in the bottom status bar.
- **Copilot Bridge: Refresh Plan Quotas & Balances** — query all active provider endpoints and update the status bar tooltip.
- **Copilot Bridge: Select Vision Agent Model** — choose which multimodal model powers automatic visual analysis for text-only coding models.
- **Copilot Bridge: Run Diagnostics & Connectivity Test** — test connectivity to all configured model endpoints and print a detailed report to the Output Channel.
- **Copilot Bridge: Show Debug Output Logs** — open the dedicated Copilot Bridge output log channel.
- **Copilot Bridge: Toggle Debug Logging** — toggle verbose debug logging on or off.
- **Copilot Bridge: Reset All Configuration & Clear Secrets** — reset all Copilot Bridge settings, clear stored secrets in SecretStorage, and clean up bridge models and companion MCP servers.
## Requirements
...
```bash
git clone https://github.com/thejjw/copilot-bridge
cd copilot-bridge
npm install
npm run package
...
- **Zero Intermediate Proxy / Zero-Telemetry**: Copilot Bridge does not proxy, intercept, or relay your chat traffic. All chat requests travel directly from VS Code to official provider endpoints over encrypted HTTPS/TLS.
...
- **Factory Reset**: You can clear all extension secrets and purge bridge configurations at any time via the Command Palette: **`Copilot Bridge: Reset All Configuration & Clear Secrets`**.
...
- **Why Plaintext on Disk Is Required in Current VS Code**:
  ...
  - Consequently, Copilot Bridge writes credentials into `chatLanguageModels.json` to enable automated setup and immediate chat execution without manual JSON hacking.
- **Exposure Scope**:
...
- **Security Best Practices & Key Rotation**:
  ...
  - Use **`Copilot Bridge: Reset All Configuration & Clear Secrets`** to purge all stored credentials and bridge definitions from your machine.
- **Future Roadmap**:
  - If VS Code introduces an extension-accessible secret bridge for Custom Endpoints, or if Copilot Bridge implements an in-process `vscode.lm.registerLanguageModelChatProvider` runtime streaming provider, credentials can be held exclusively in memory and `context.secrets`.
